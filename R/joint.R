...
#'
#' Joint measures consider the prediction of all events within a fixed target vector.
...
#' @param target Target trait vector for evaluating joint measures of accuracy.
#' If NULL the marginal measures are calculated across the distribution of trait vectors.
#' @param target2 Second target vector for calculating joint concordance.
#' @param targetProb Probability of all events occurring in \code{target}, used for calculating net benefit.
#' @inheritParams eventWise
#'
#' @export
joint=function(x,y,thresh=NULL,target=NULL,target2=NULL,nsample=0,targetProb=NULL) {
  ...
  # sensitivity
  sens = mean(y[targetIndex,] %*% t(1-predictedTrait[targetIndex,])==0)

  # specificity
  spec = mean((1-y[targetIndex,]) %*% t(predictedTrait[targetIndex,])==0)

  # rows of predictedTrait matching target
  targetPredictedTrait = apply(predictedTrait,1,function(x) sum(x!=target)==0)

  # positive predictive value
  PPV = mean(predictedTrait[targetPredictedTrait,] %*% t(1-y[targetPredictedTrait,])==0)

  # negative predictive value
  NPV = mean((1-predictedTrait[targetPredictedTrait,]) %*% t(y[targetPredictedTrait,])==0)
  ...
  # net benefit
  # probability of all events of the target vector
  if (is.null(targetProb)) {
    ...
    for(i in 1:dim(y)[1]) {
      ...
      yMatch = yMatch + (sum(y[i,]!=target) == 0)
    }
  ...
  familyWiseSpec = mean((1-y[cTargetIndex,]) %*% t(1-predictedTrait[cTargetIndex,])>0)
